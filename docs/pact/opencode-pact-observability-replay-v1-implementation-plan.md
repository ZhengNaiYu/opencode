...
- LoLBench/OpenCode smoke must inject the ZAI Coding Plan custom provider from env placeholders instead of treating `zai-coding-plan/*` as a built-in registry model. The local default config path declares `models.glm-5.2` with `@ai-sdk/openai-compatible`, `ZAI_API_BASE`, and `ZAI_API_KEY`.
...
- [x] Declare `models.glm-5.2` in the local PACT/OpenCode config path; `opencode run -m zai-coding-plan/glm-5.2` is the verified local default invocation.
