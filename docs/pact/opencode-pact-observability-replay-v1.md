...
- Deterministic LoLBench spec imports use `planner_backend=spec-import`, `planner_model=null`, and the same Round00 resume driver path as planner-generated Round00 packages. Imported loops expose `spec-input-manifest.json`, `spec-source/enhanced_requirement_sections`, and `spec-code-localization.md` when present. The local index is `/Users/gujiazhen/Documents/cc_codes/outputs/lolbench_pact_round0_imports_v14_opencode_best/round0-imports.tsv`.
